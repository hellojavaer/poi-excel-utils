...
public class ExcelUtils {
	...
	static {
		...
		try {
			TIME_1899_12_31_00_00_00_000 = df.parse("1899-12-31 00:00:00:000")
					.getTime();
			TIME_1900_01_01_00_00_00_000 = df.parse("1900-01-01 00:00:00:000")
					.getTime();
			TIME_1900_01_02_00_00_00_000 = df.parse("1900-01-02 00:00:00:000")
					.getTime();
	...
	private static void readConfigParamVerify(
			ExcelReadSheetProcessor<?> sheetProcessor) {
		Class<?> clazz = sheetProcessor.getTargetClass();
		ExcelReadFieldMapping fieldMapping = sheetProcessor.getFieldMapping();
		for (Entry<Integer, Map<String, InnerReadCellProcessorWrapper>> indexFieldMapping : fieldMapping
				.entrySet()) {
			for (Map.Entry<String, InnerReadCellProcessorWrapper> filedMapping : indexFieldMapping
					.getValue().entrySet()) {
				...
				if (fieldName != null) {
					...
					if (pd == null || pd.getWriteMethod() == null) {
						throw new IllegalArgumentException(
								"In fieldMapping config {colIndex:"
										+ indexFieldMapping.getKey()
										+ "["
										+ convertColIntIndexToCharIndex(indexFieldMapping
												.getKey()) + "]<->fieldName:"
										+ filedMapping.getKey() + "}, "
										+ " class " + clazz.getName()
										+ " can't find field '"
										+ filedMapping.getKey()
										+ "' and can not also find "
										+ filedMapping.getKey()
										+ "'s writter method.");
					}
					if (!Modifier.isPublic(pd.getWriteMethod()
	...
	public static void readInputStreamToObject(InputStream workbookInputStream,
		...
		Assert.isTrue(workbookInputStream != null,
				"workbookInputStream can't be null");
		Assert.isTrue(sheetProcessors != null && sheetProcessors.length != 0,
				"sheetProcessor can't be null");
		try {
			...
			for (ExcelReadSheetProcessor<?> sheetProcessor : sheetProcessors) {
				...
				try {
					...
					if (sheetName != null) {
						...
						throw new IllegalArgumentException(
								"sheetName or sheetIndex can't be null");
					}
					if (sheet == null) {
						throw new IllegalArgumentException(
								"Sheet Not Found Exception. for sheet name:"
										+ sheetName);
					}
					...
					if (sheetProcessor.getPageSize() != null) {
						context.setDataList(new ArrayList(sheetProcessor
								.getPageSize()));
					} else {
					...
					if (pageSize != null) {
						...
						for (int i = 0; i < pageCount; i++) {
							...
							readInputStreamToObject(context, sheet, start,
									size, sheetProcessor.getFieldMapping(),
						...
						readInputStreamToObject(context, sheet, startRow,
								rowEndIndex - startRow + 1,
	...
	private static <T> void readInputStreamToObject(
			ExcelReadContext<T> context, Sheet sheet, int startRow,
		...
		Assert.isTrue(pageSize == null || pageSize >= 1,
	...
	private static <T> T readRow(ExcelReadContext<T> context, Row row,
		...
		for (Entry<Integer, Map<String, InnerReadCellProcessorWrapper>> fieldMappingEntry : fieldMapping
				.entrySet()) {
			...
			if (curColIndex > lastColIndex || curColIndex < minColIx) {
				Map<String, InnerReadCellProcessorWrapper> fields = fieldMappingEntry
						.getValue();
				for (Map.Entry<String, InnerReadCellProcessorWrapper> field : fields
						.entrySet()) {
				...
				Map<String, InnerReadCellProcessorWrapper> fields = fieldMappingEntry
						.getValue();
				for (Map.Entry<String, InnerReadCellProcessorWrapper> fieldEntry : fields
						.entrySet()) {
					...
					PropertyDescriptor pd = org.springframework.beans.BeanUtils
							.getPropertyDescriptor(targetClass, fieldName);
					if (pd == null || pd.getWriteMethod() == null) {
						continue;
					}

					Object value = _readCell(cell);
					value = procValueConvert(context, row, cell, entry,
							fieldName, value);
					if (value != null) {// ignore null
						try {
							Class<?> paramType = pd.getWriteMethod()
									.getParameterTypes()[0];
							if (value != null
									&& !paramType.isAssignableFrom(value
											.getClass())) {
								value = TypeUtils.cast(value, paramType, null);
							}
							pd.getWriteMethod().invoke(context.getCurRowData(),
									value);
						} catch (Exception e1) {
	...
	private static Object procValueConvert(ExcelReadContext<?> context,
			Row row, Cell cell, InnerReadCellProcessorWrapper entry,
		...
		if (entry.getValueMapping() != null) {
			...
			if (convertedValue == null) {
				if (!valueMapping.containsKey(strValue)) {
					if (valueMapping.isSetDefaultValue()) {
						...
						try {
							convertedValue = valueMapping.getDefaultProcessor()
									.process(context, cell,
						...
						if (convertedValue != null
								&& convertedValue instanceof ExcelCellValue) {
							convertedValue = value;
						}
					} else {
						ExcelReadException e = new ExcelReadException();
			...
			try {
				convertedValue = entry.getProcessor().process(context, cell,
						new ExcelCellValue(value));
			} catch (RuntimeException re) {
				if (re instanceof ExcelReadException) {
					ExcelReadException ere = (ExcelReadException) re;
	...
	public static void writeObjectToOutputStream(InputStream template,
	...
	public static void writeObjectToOutputStream(ExcelType fileType,
			OutputStream outputStream,
			ExcelWriteSheetProcessor<?>... sheetProcessors) {
		...
		writeObjectToOutputStream(false, workbook, outputStream,
				sheetProcessors);
	}
	...
	private static void writeObjectToOutputStream(boolean useTemplate,
			Workbook workbook, OutputStream outputStream,
			ExcelWriteSheetProcessor<?>... sheetProcessors) {
				...
				dataList = sheetProcessor.getDataList(context)) {
					for (Object rowData : dataList) {
						...
						if (templateRow != null) {
							...
							row.setHeightInPoints(templateRow
									.getHeightInPoints());
						...
						// ///////
						if (rowData != null) {
							writeRow(context, templateRow, row, rowData,
									sheetProcessor);
						}

						Row reRow = null;
						try {
							reRow = sheetProcessor.process(context, rowData,
									row);
						} catch (RuntimeException e) {
							if (e instanceof ExcelWriteException) {
								...
								ExcelWriteException ewe = new ExcelWriteException(
										e);
	...
	@SuppressWarnings("rawtypes")
	private static void writeDataValidations(Sheet sheet,
		...
		for (Entry<String, Map<Integer, InnerWriteCellProcessorWrapper>> fieldIndexMapping : sheetProcessor
				.getFieldMapping().entrySet()) {
			if (fieldIndexMapping == null
					|| fieldIndexMapping.getValue() == null) {
				continue;
			}
			for (Entry<Integer, InnerWriteCellProcessorWrapper> indexProcessorMapping : fieldIndexMapping
					.getValue().entrySet()) {
				if (indexProcessorMapping == null
						|| indexProcessorMapping.getKey() == null) {
					continue;
				}
		...
		List<? extends DataValidation> dataValidations = sheet
				.getDataValidations();
		if (dataValidations != null) {
			for (DataValidation dataValidation : dataValidations) {
				...
				CellRangeAddressList cellRangeAddressList = dataValidation
						.getRegions();
				if (cellRangeAddressList == null) {
					continue;
				}

				CellRangeAddress[] cellRangeAddresses = cellRangeAddressList
						.getCellRangeAddresses();
				if (cellRangeAddresses == null
						|| cellRangeAddresses.length == 0) {
					continue;
				}
				...
				for (CellRangeAddress cellRangeAddress : cellRangeAddresses) {
					...
					for (Integer configColIndex : configColIndexSet) {
						if (configColIndex < cellRangeAddress.getFirstColumn()
								|| configColIndex > cellRangeAddress
										.getLastColumn()) {// specify column
							continue;
						}
						newCellRangeAddressList.addCellRangeAddress(
								rowStartIndex, configColIndex,
								sheet.getLastRowNum(), configColIndex);
						validationContains = true;
					}
				}
				if (validationContains) {
					DataValidation newDataValidation = sheet
							.getDataValidationHelper().createValidation(
	...
	private static void writeRow(ExcelWriteContext context,
			InnerRow templateRow, Row row, Object rowData,
			ExcelWriteSheetProcessor sheetProcessor) {
		...
		for (Entry<String, Map<Integer, InnerWriteCellProcessorWrapper>> entry : fieldMapping
				.entrySet()) {
			String fieldName = entry.getKey();
			Map<Integer, InnerWriteCellProcessorWrapper> map = entry.getValue();
			for (Map.Entry<Integer, InnerWriteCellProcessorWrapper> fieldValueMapping : map
					.entrySet()) {
				Integer colIndex = fieldValueMapping.getKey();
				InnerWriteCellProcessorWrapper cellProcessorWrapper = fieldValueMapping
						.getValue();
				...
				ExcelWriteCellValueMapping valueMapping = cellProcessorWrapper
						.getValueMapping();
				ExcelWriteCellProcessor processor = cellProcessorWrapper
						.getProcessor();
				if (valueMapping != null) {
					...
					if (cval != null) {
						writeCell(row.getRowNum(), colIndex, cell, cval,
								useTemplate);
					} else {
						if (!valueMapping.containsKey(key)) {
							if (valueMapping.isSetDefaultValue()) {
								if (valueMapping
										.isSetDefaultValueWithDefaultInput()) {
									writeCell(row.getRowNum(), colIndex, cell,
											val, useTemplate);
								} else {
									writeCell(row.getRowNum(), colIndex, cell,
											valueMapping.getDefaultValue(),
											useTemplate);
								}
							} else if (valueMapping.getDefaultProcessor() != null) {
								cell = valueMapping.getDefaultProcessor()
										.process(context, rowData, cell);
							} else {
	...
	private static void writeCell(int rowIndex, int colIndex, Cell cell,
			Object val, boolean userTemplate) {
		try {
	...
	public static void writeCell(Cell cell, Object val) {
		if (cell.getCellStyle() != null
				&& cell.getCellStyle().getDataFormat() > 0) {
	...
	private static void writeCell(Cell cell, Object val, boolean userTemplate) {
		...
		if (val instanceof Byte) {// Double
			...
			// 1900/01/01
			if (time >= TIME_1899_12_31_00_00_00_000
					&& time < TIME_1900_01_01_00_00_00_000) {
			...
			if (!userTemplate) {
				...
				DataFormat dataFormat = wb.getCreationHelper()
						.createDataFormat();
				// @see #BuiltinFormats
				// 0xe, "m/d/yy"
				// 0x14 "h:mm"
				...
				if (time >= TIME_1899_12_31_00_00_00_000
						&& time < TIME_1900_01_02_00_00_00_000) {
