...
public class ExcelReadFieldMapping implements Serializable {
	...
	public void put(int colIndex, String fieldName,
			ExcelReadCellProcessor proccessor) {
	...
	public void put(int colIndex, String fieldName,
			ExcelReadCellValueMapping valueMapping, boolean required) {
	...
	private void put(int colIndex, String fieldName,
			ExcelReadCellValueMapping valueMapping,
			ExcelReadCellProcessor proccessor, boolean required) {
		Map<String, InnerReadCellProcessorWrapper> map = fieldMapping
				.get(colIndex);
		if (map == null) {
		...
		map.put(fieldName, new InnerReadCellProcessorWrapper(valueMapping,
				proccessor, required));
	}

	private void put(String colIndex, String fieldName,
			ExcelReadCellValueMapping valueMapping,
			ExcelReadCellProcessor proccessor, boolean required) {
		put(ExcelUtils.convertColCharIndexToIntIndex(colIndex), fieldName,
				valueMapping, proccessor, required);
	}
